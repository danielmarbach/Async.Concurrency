namespace NServiceBus.AzureServiceBus
{
    ...
    // - Lock-free
    // - Allocates the necessary collections up front for the maxed-out scenario. The only allocations you pay during runtime is a Tuple and the async state machine
    ...
    class MultiProducerConcurrentCompletion<TItem>
    {
        ...
        void PushInBatchesUpToConcurrencyPerQueueForAGivenSlot(ConcurrentQueue<TItem> queue, int currentSlotNumber, List<Task> tasks) {
            ...
            do {
                ...
                }, ValueTuple.Create(items, itemListBuffer), TaskContinuationOptions.ExecuteSynchronously);
                tasks.Add(task);
